...
indicate the end of the class list. Just like in Ruby, any exceptions not in
this list will not be rescued. If you just want to rescue `StandardError` (like
a blank `rescue` in Ruby), you can use `rb_rescue()` which uses just the first
...
it returns `Qnil` and sets `state` to some nonzero value.
...
Note that just like in Ruby, the return value of `ensure_func()` is never used.
If no exception occurs, `rb_rescue()` will return the value of `begin_func()`
...
method name, etc. A general warning though: unlike Ruby, when you give
something an invalid name **the API won't complain**. Ruby will rase a
`SyntaxError` or `NameError` if you try to name a class `foo` (not constant) or
an instance variable `bar` (with no `@`), but the API will happily create them.
The API handles this by **not exposing invalid names to Ruby**.  Since that's
generally not what you want, double check the names you choose!
...
Note that the `VALUE` should be initialized before you create the global in Ruby
and it should be global in C as well &ndash; you don't want it to go out of
scope while Ruby is using it! For `rb_define_readonly_variable()` the object
is not frozen, so it can still be modified from Ruby if it is mutable. For
`rb_define_hooked_variable()`, you can pass `0` for the getter/setter if you
don't need both functions.

### Instance Variables ###

You can get/set instance variables just like globals, but you must use an `ID`
like with `rb_funcall()`.

{% highlight c %}
{% include ivar.snip %}
{% endhighlight %}

There isn't an automatic way to synchronize like with globals.

### Constants ####

Constants work similarly, but you must specify the module to define them under:
