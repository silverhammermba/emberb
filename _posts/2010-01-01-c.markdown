...
for the `VALUE`, `TYPE()` will return `T_OBJECT`[^tdata] (which makes sense
since everything in Ruby `is_a?  Object`). That's why the `default:` for the
above switch should not normally be reached. Considering raising an
...
`false`, and `true` are prefixed with `Q` e.g. `Qnil`. As a convenience,
`Qfalse` is also false in C (`0`).[^undef]
...
this [isn't a bug][bug]). Also, `NUM2CHR()` actually calls `NUM2INT()`, so it
...
initilization, you can just wrap your data in a `struct` and do the _actual_
...
example, you can pass `RUBY_DEFAULT_FREE` as the function pointer and Ruby will
...
`Data_Make_Struct()` does the allocation for you _and_ wraps it. We could've
...
[^tdata]: Or it could return `T_DATA` if the object [wraps C data](#data).
