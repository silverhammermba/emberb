...
Getting/setting instance variables requires an object to store the variable and
you must use an `ID` like with `rb_funcall()`.
...
defining methods, but you'll need the same three things for pretty much all of
them: the `VALUE` for the class getting the method, a `char*` for the method
name, and a C function which the method will call. The function should always
return a `VALUE`, but there are three ways you can define its arguments:
