...
binding &ndash; like when you `require` something. So local variables in the
...
`PRIsVALUE` is defined to some variant of the `%i` conversion specifier, so when
printing `int`s you should probably stick to `%d` just to be safe.
...
Head over to the [Exampels](../examples) page for short, compilable examples of
...
[^wrap]: There's also `rb_eval_string_wrap()` which _should_ be useful, but is
         actually the same as `rb_eval_string_protect()` due to a [bug].
...
[bug]: https://bugs.ruby-lang.org/issues/10466
