...
containg nulls, use `rb_str_new()` which takes a `char*` and the string's length
(as a `long`)[^utf].
...
it returns `Qnil` and sets `state` to some nonzero value. If you want to reraise
the exception, pass `state` to `rb_jump_tag()` (this works for
`rb_eval_string_protect()` too).
...
things invalid names**. Ruby will rase a `SyntaxError` or `NameError` if you try
to name a class `foo` (not constant) or an instance variable `bar` (with no
`@`), but the API will happily create them.  The API handles this by **not
exposing invalid names to Ruby**. Since that's probably not what you want,
double check the names you choose!
...
Ruby is using it! For `rb_define_readonly_variable()` the object is not frozen,
so it can still be modified from Ruby if it is mutable. For
`rb_define_hooked_variable()`, you can pass `0` for the getter/setter if you
want to synchronize normally for that operation. Or you can throw out `global`
entirely with `rb_define_virtual_variable()` though of course the getter and
setter _must_ be defined in that case.
...
first argument must be a class. Surprising.
...
the method will call. The function must return a `VALUE`, and there are three
ways you can define its arguments:
...
fixed number of arguments[^self], and ones that slurp up all their arguments.
What about all of Ruby's fancy argument features? Where are optional args,
keyword args, options hashes, blocks, and all the mixtures of those?
...
Now that you've got your C function set up, it's a simple matter of creating the
method in Ruby. Every API call takes (among other things) the method name
(`char*`), a pointer to your C function, and an `argc` describing its arguments.
`argc` should be:

1. For a fixed number of arguments, the number of arguments
2. For variable args in a C array, `-1`
3. For variable args in a Ruby Array, `-2`
...
"offical" API documentation. It's included in the Ruby source as a plain text
...
[^self]: Though how does this count the `self` argument?
