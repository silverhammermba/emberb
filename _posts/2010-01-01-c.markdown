...
TODO when was the TypedData\* stuff introduced?
...
use the `TypedData_Wrap_Struct()` macro to wrap the pointer in a `VALUE`. The
first argument is the class to give the `VALUE`, which here is the receiver
since we're in a class method. The second argument is a pointer to a struct
defining several options for use internally by Ruby and the last argument is the
data pointer itself. The meaning of the fields in that struct are:

* `wrap_struct_name` is a string used by Ruby to internally identify your type.
  It doesn't really matter what it is as long as it's sensible and unique
...
TODO test this marking shit too
