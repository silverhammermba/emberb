...
To raise an exception in one function but compose the exception message in
another function (e.g. if you have a helper function just for raising
the exception), use `rb_exc_raise` with `rb_exc_new_str`:

{% highlight c %}
{% include c/raise_exc_str.h %}
{% endhighlight %}

If the exception message is in a NULL-terminated C string and no printf-style
formatting is needed, you could use `rb_exc_new_cstr`:

{% highlight c %}
{% include c/raise_exc_cstr.h %}
{% endhighlight %}

If the exception message is a binary blob, use `rb_exc_new` which takes the
length of the blob as the third argument:

{% highlight c %}
{% include c/raise_exc_new.h %}
{% endhighlight %}
