...
`void*`, you may want to define a `struct` for passing data via pointers. Also,
the last two arguments don't _have_ to be `NULL` but they're ignored, so why
not?

The downside of the previous approach is that `slow_func` can't be interrupted[^int]
(by signals, `Thread.kill`, etc.). To allow for that you use a slightly
...
[^int]: The documentation in `thread.c` says "If `rb_thread_call_without_gvl2()`
        detects interrupt, it returns immediately." but this wasn't the case in
		my tests. It would wait until it returns normally and _then_ raise the
		interrupt.
