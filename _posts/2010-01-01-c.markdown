...
use the `Data_Wrap_Struct()` macro to wrap the pointer in a `VALUE`. The first
argument is the class to give the `VALUE`, which here is the receiver since
we're in a class method. The second argument is used by Ruby's garbage collector
and will be described later, but as long your C data doesn't point to any Ruby
objects you don't need it and can pass `NULL`. The third argument is the
function that the garbage collector will call when the object is destroyed, and
last is the data pointer itself. `VALUE`s that wrap C data will have type
`T_DATA` with respect to the [`TYPE()` macro](#value).
