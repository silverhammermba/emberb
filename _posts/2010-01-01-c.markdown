...
no Ruby analogs (e.g. data structures defined by some C library)?
...
This simple example creates the class `Foo` to wrap `int`s (in most cases you'll
probably be wrapping something more complicated, like a `struct`). After
allocating the C data, we use the `Data_Wrap_Struct()` macro to wrap the pointer
in a `VALUE`. The first argument is the class to give the `VALUE`, which here is
the receiver since we're in a class method. The second argument is used by
Ruby's garbage collector and will be described later, but as long your C data
doesn't point to any Ruby objects you don't need it and can pass `NULL`.  The
third argument is the function that the garbage collector will call when the
object is destroyed, and last is the data pointer itself. `VALUE`s that wrap C
data will have type `T_DATA` with respect to the [`TYPE()` macro](#value).
...
free your data just calls `free()` as in the example, you can have Ruby call
`free()` for you by passing `-1` for the function pointer (**don't** use `NULL`
unless you like memory leaks). Similarly, if your allocation is just a
`malloc()` as in the example, the macro `Data_Make_Struct()` does both the
allocation and wrapping at once. We could've rewritten the allocation function
as such:
