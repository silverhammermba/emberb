...
C string API calls, you may see some performance benefit by storing the `ID` and
using the `ID` versions of the functions (though you'll have to look these up
yourself in the Ruby headers).
...
You can call `rb_iter_break(void)` or `rb_iter_break_value(VALUE)` to break out
of a block.
...
avoid it being prematurely cleaned up:
...
equivalent to `Proc.new` which converts the method block to a proc:

    VALUE block;
	block = rb_block_proc();

There are a few ways to yield to a block:

{% highlight c %}
{% include yield.h %}
{% endhighlight %}

There's also `rb_yield_values2()` which is like `rb_yield_values()` but the
second argument is a `VALUE*` rather than being a varargs[^yield].
