...
You don't "need" to know any advanced C concepts in order to use the API; most
of the complexity is hidden behind functions and macros. However the API is
**huge** and largely **undocumented**. After you start using it, you will
inevitably find yourself delving through the Ruby source code at some point to
figure out the behavior of some obscure function or macro. The Ruby source uses
some fairly sophisticated C, so you'll need to at least be able to read it.
...
code (all Ruby code returns something) and sets `state` to some nonzero value if
any exception is raised. `VALUE` is the C data type for all Ruby objects, as
...
point of writing in C. If you just want to call some Ruby method, we'll go over
a better way to do that [later on](#send).
...
Ruby objects (much like variables in Ruby contain pointers to objects). These
...
If you instead want the `VALUE` of the class object, use the `CLASS_OF()` macro.
...
whatever `Numeric` to the desired C type, and raise a `RangeError` if there is
...
this [isn't a bug][bug]). Also, `NUM2CHR()` really just calls `NUM2INT()`, so it
won't raise if the value would fit an `int` but not a `char`.
...
C function to represent the block[^break]. Then there's a different variant of
...
arguments from `argv`, so why not play it safe?
...
If you ever create a global `VALUE` in C which is _not_ exposed to Ruby (using
one of the previous functions), you must tell the garbage collector about it to
prevent it from being prematurely cleaned up:
...
You define constants similarly, but with the module to define them under:
...
depends on what you want to happen if the constant is _not_ defined in the
module you specify:
...
you don't need. Be aware that the parsing of the format string is greedy: `1*`
...
pass `0` for an argument you don't care about. For example the format string
...
Unlike in Ruby `rb_call_super()` will **not** implicty pass along the method
arguments to the super if you give it no arguments. You must explicitly pass the
correct `argc` and `argv`. For that reason I recommend using the third (C array)
style of method definition if you want to use `rb_call_super()`.
...
your data can be naturally translated into `VALUE`s it's easy: just create
instance variables and assign them `VALUE`s as usual. But what if your data have
no Ruby analog (e.g. data structures defined by some C library)?
...
whenever you need the C data, you can unpack the pointer and cast it back to the
correct type. But where does this encapsulation occur? Let's answer that
question with a question: what happens when you tell Ruby to create an object
using `new`?  Basically this:
...
first calls the _class method_ `allocate` in order to actually create the
...
This simple example create the class `Foo` which wraps an `int` that can be set
by `initialize` (in most cases you'll probably be wrapping something more
complicated, like a `struct`). After allocating the C data, we use the
`Data_Wrap_Struct()` macro to wrap the pointer in a `VALUE`. The first argument
is the class to give the `VALUE`, which here is the receiver since we're in a
class method. The second argument is used by Ruby's garbage collector and will
be described later, but as long your C data doesn't point to any Ruby objects
you don't need it and can pass `NULL`.  The third argument is the function that
the garbage collector will call when the object is destroyed, and last is the
data pointer itself. `VALUE`s that wrap C data will have type `T_DATA` with
respect to the [`TYPE()` macro](#value).

`Data_Get_Struct()` is simpler: it takes the object to unwrap, the type of the
underlying data, and the pointer to assign the data to.

The previous example can actually be simplified a little. If the function to
free your data just calls `free()` as in the example, you can pass
`RUBY_DEFAULT_FREE` as the function pointer and Ruby will free it for you
(**don't** use `NULL` unless you like memory leaks). Similarly, if your
allocation is just a `malloc()` as in the example, the macro
`Data_Make_Struct()` does both the allocation and wrapping at once. We could've
rewritten the allocation function as such:
...
That `NULL` argument we passed to the wrapping macros is the pointer to your
object's "mark function". This is so named because of the garbage collector's
"mark and sweep" algorithm. The basic idea behind mark and sweep is that when
the garbage collector needs to free up memory, it performs two passes: the first
(mark) pass iterates through every _referenced_ Ruby object and marks it as
active, then the second (sweep) pass iterates through every _allocated_ Ruby
...
C `struct` which contains a Ruby `VALUE` &ndash; which the garbage collector is
...
collector needs to free up memory, your C data is going to end up with an
invalid reference to a Ruby object.

In the following example, we'll wrap a C struct which contains a `VALUE`. The
mark function has the same signature as the free function:
...
The cast to `void*` is incidental &ndash; the function immediately casts it back
to `VALUE`. See the other thread functions in `ruby/intern.h` (but there's
always `rb_funcall()` for everything else).
...
you'll often hear people say that `Thread` is not truly concurrent.
...
`Thread` because **it will block all other threads** while it runs.  However if
you have some C code that doesn't need the API, you can tell the VM to release
the GVL before running your code in a thread and to reacquire it when it
completes, allowing for true concurrency. Locking and unlocking the GVL does
carry a performance hit, so only resort to this if you notice that you're having
significant problems due to blocked threads.
...
`void*`, you may want to define a `struct` for passing data via pointers. So now
your code will run concurrently in `Thread`s, but you'll find that it can't be
interrupted (by signals, `Thread.kill`, etc.)! To allow for that you can use the
last two arguments:
...
[README.EXT](../readme) is Ruby's official API documentation. It's included in
the Ruby source as a plain text file, but I've HTML-ified it for nicer online
reading. It has a bit more information than this guide, but in most cases that's
because I intentionally omitted something that I either found not useful or
...
[^intern]: Okay, that's a blatant lie. The API _totally_ lets you mess around
           ...
           names starting with capital R). It's just generally not a good idea
           and not necessary.

[^undef]: There's also `Qundef` representing an undefined value, but
          this has no Ruby equivalent and is [rarely](#handling-blocks) used.

[^wrap]: There's also `rb_eval_string_wrap()` but I'm not entirely sure what the
         use-case is for it.
...
[^pi]: The documentation [claims](../readme#exceptions-and-errors) that the
       format directives `%i` and `%+i` can be used for `to_s` and `inspect`
       (both take a `VALUE`) but a glance at the source shows that this isn't
       true. `%i` is the same as `%d`.
...
[^int]: There is also the function `rb_thread_call_without_gvl2()`. The
        ...
        unblocking function doesn't kill the thread, it still has to wait for
        the thread to finish on its own before returning.
