...
difference is that the message is a format string just like in `printf`[^4],
...
clear `$!` after a rescue so you need to clear it manually[^5]. Otherwise later API
calls might read the old value and think an exception has occurred.
...
[^3]: Again, the documention [mentions](../readme#string-functions) that there
