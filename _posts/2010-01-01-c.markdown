...
When it comes to catching exceptions, it matters [what approach](#the-two-paths)
you're taking. If you're compiling a library to be loaded by Ruby, you've got it
easy: exceptions raised in the API can be caught as usual by your Ruby code. If
you're embedding Ruby in C/C++, you need to be _extremely careful_ when calling
API functions that could raise an exception because **an uncaught exception will
segfault and kill your program**. So, how do we rescue?

{% highlight c %}
{% include rescue.snip %}
{% endhighlight %}

First you need to wrap the dangerous code in a function that takes a `VALUE`
argument and returns a `VALUE`. The argument and return values have no specific
use so you can just ignore them if you like. Then you pass the function pointer
and its argument to `rb_protect()` along with a state pointer. Just like for
`rb_eval_string_protect()`, this function returns the normal return value if no
exception is raised, otherwise it returns `Qnil` and sets `state` to a nonzero
value. If `state` is nonzero, you can retrieve the exception object with
`rb_errinfo()`. You must also clear the exception with `rb_set_errinfo(Qnil)` or
else later API calls might think that an exception occurred[^5].

If you would rather handle the exception in a separate function (if you want to
reuse your rescue code, for example), you can use `rb_rescue()` which calls
another function when an exception is raised. `rb_ensure()` is like
`rb_rescue()` except the second function is _always_ called and **no exceptions
are caught**. In order to get something like Ruby's `begin; rescue; ensure; end`
...
Note that all of these approaches rescue _every_ exception (which is usually not
what you want to do). Your rescue code should check the exception type and
reraise[^6] if it isn't some expected type. Or you can use `rb_rescue2()` which
is a varargs function taking a list of exception classes to catch.
...
[^5]: The documentation [states](../reamde#control-structure) that "You have to
	 ...
	 aware of any case where it would be cleared for you &ndash; it seems like
	 you _always_ have to clear it.

[^6]: Is there a shortcut for raising an exception instance?
