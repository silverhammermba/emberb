...
by sending the object `to_s`. You can substitute the result of `inspect` by
using the `+` flag with the specifier.
...
thread and to reacquire it when it completes, allowing for true concurrency.
...
concurrently, it can't be interrupted (by signals, `Thread.kill`, etc.)! To
